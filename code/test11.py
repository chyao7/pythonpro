...
分为 5 等：用整数 1 ~ 5 表示，第 5 等最重要。他还从因特网上查到了每件物品的价格
（都是 10 元的整数倍）。他希望在不超过 N 元（可以等于 N 元）的前提下，
...
for i in range(1,m+1):
    for j in range(1,n+1):
        res[i][j] = res[i-1][j]
        if j>p[i-1] and res[i][j]<res[i-1][j-w[i-1]]+p[i-1]*w[i-1]:
            if s[i-1]==0:
                res[i][j]=max(res[i-1][j-p[i-1]]+p[i-1]*w[i-1],res[i][j])
            else:
                res[i][j]=max(res[i-1][j-p[i-1]],res[i][j])
