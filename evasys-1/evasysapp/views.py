...
def resvisualization(request):
    if request.method == "GET":
        if request.GET.get("id",None):
            ...
            valResInstance = models.val_res.objects.get(val_res_id=val_res_id)
            companyName = models.company.objects.get(company_id=valResInstance.company_id).company_name
            algorithmName = models.Algorithm_description.objects.get(algorithm_id=valResInstance.algorithm_id).algorithm_name
            totalRes, periodRes, evaluatorRes, seniorRes = valResInstance.final_res, valResInstance.period_res, valResInstance.evaluator_res, valResInstance.senior_res
            periodResDict, evaluatorResDict, seniorResDict = json.loads(periodRes),json.loads(evaluatorRes),json.loads(seniorRes)
            valModelId = valResInstance.val_model_id
            evaData = models.val_model.objects.get(val_model_id=valModelId).data
            evaDataJson = json.loads(evaData)
            jingDianYuId = models.val_model.objects.get(val_model_id=valModelId).interval_id
            jingDianYu = models.interval.objects.get(interval_id=jingDianYuId).interval_data
            jingDianYuList = list(map(int, jingDianYu.split(",")))
            jingDianYuTuple = tuple((jingDianYuList[i], jingDianYuList[i + 1]) for i in range(len(jingDianYuList) - 1))
            mapJingDianYuList = [1 if float(totalRes)<=x[1] and float(totalRes)>x[0] else 0 for x in jingDianYuTuple]
            mapScoreDict = {0:"很差", 1:"较差", 2:"一般", 3:"较好", 4:"很好"}
            index = np.argmax(mapJingDianYuList)
            totalEvaluation = mapScoreDict[index]
            return render(request, "resvisualization.html",
                          {"totalRes": totalRes, "totalEvaluation": totalEvaluation,
                           "periodResDict": periodResDict,
                           "evaluatorResDict": evaluatorResDict, "seniorResDict":seniorResDict,
                           "companyName":companyName, "algorithmName":algorithmName, "val_res_id":val_res_id})
        else:
            return HttpResponse("BAD REQUEST")
    elif request.method == "POST":
        name = request.POST.get("senior_name",None)
        val_res_id = request.POST.get("val_res_id",None)
        obj = models.val_res.objects.get(val_res_id=val_res_id)
        senior_index = pd.read_json(obj.senior_res)
        senior_index = senior_index.sort_values(by='senior_index_id')
        junior_res = pd.read_json(obj.junior_res)
        junior_res = junior_res.T
        junior_res['senior_index_id'] = junior_res['senior_index_id'].astype('int64')
        def senior_plot(senior_name, senior_index, junior_res):
            if senior_name not in senior_index['senior_index_name'].values:
                print('非法指标')
                return 0
            index = senior_index[senior_index['senior_index_name'] == senior_name].index
            title = senior_name + ':' + str(
                round(senior_index.loc[index, 'value'].values[0], 2))
            senior_id=senior_index.loc[index, 'senior_index_id'].values
            df = junior_res[junior_res['senior_index_id'] == senior_id[0]]
            plt.figure(figsize=(12, 9))
            labels = df[['junior_index_name', 'value']].values
            sizes = [round(x, 2) for x in df['value'].values]
            labels1 = []
            labels2 = df['junior_index_name']
            for i in labels:
                labels1.append(i[0] + ':' + str(round(i[1], 2)))
            explode = None
            plt.pie(sizes, explode=explode, labels=labels1,
                    labeldistance=1.1,
                    shadow=False, startangle=90, pctdistance=0.8, textprops={'fontsize': 15})
            plt.title(title, fontsize=20)
            plt.axis('equal')
            plt.legend(labels2, fontsize=10,ncol=1,loc='upper right',bbox_to_anchor=(1.1,1.05),borderaxespad=0.3)
            sio = BytesIO()
            plt.savefig(sio, format='png')
            data = base64.encodebytes(sio.getvalue()).decode()
            return data

        data = senior_plot(name, senior_index, junior_res)
